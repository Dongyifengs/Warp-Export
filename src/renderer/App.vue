<template>
  <div v-if="ui" class="relative">
    <div class="flex justify-between">
      <div>
        <el-button type="primary" :icon="state.status === 'init' ? 'milk-tea': 'refresh-right'" class="focus:outline-none" :disabled="!allowClick()" plain @click="fetchData()" :loading="state.status === 'loading'">{{state.status === 'init' ? `加载数据`: `更新数据`}}</el-button>
        <el-button icon="folder-opened" @click="saveExcel" class="focus:outline-none" :disabled="!gachaData"  type="success" plain>导出Excel</el-button>
        <el-tooltip v-if="detail && state.status !== 'loading'" content="从其他账号导出数据" placement="bottom">
          <el-button @click="newUser()" plain icon="plus"  class="focus:outline-none"></el-button>
        </el-tooltip>
        <el-tooltip v-if="state.status === 'updated'" content="更新已完成,点击按钮重启工具后生效" placement="bottom">
          <el-button @click="relaunch()" type="success" icon="refresh"   class="focus:outline-none" style="margin-left: 48px">直接更新</el-button>
        </el-tooltip>
      </div>
      <div class="flex gap-2">
        ...
        <el-dropdown @command="optionCommand"  >
          <el-button @click="showSetting(true)" class="focus:outline-none" plain type="info" icon="more"  >选项</el-button>
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="setting" icon="setting">设置</el-dropdown-item>
              <el-dropdown-item :disabled="!allowClick() || state.status === 'loading'" command="url" icon="link">输入URL</el-dropdown-item>
              <el-dropdown-item :disabled="!allowClick() || state.status === 'loading'" command="proxy" icon="position">代理模式</el-dropdown-item>
            </el-dropdown-menu>
          </template>
        </el-dropdown>
      </div>
    </div>
    <p class="text-gray-400 my-2 text-xs">{{hint}}<el-button @click="(state.showCacheCleanDlg=true)" v-if="state.authkeyTimeout" style="margin-left: 8px;" size="small" plain round>解决办法</el-button></p>
    ...
    <el-dialog :title="手动输入URL" v-model="state.showUrlDlg" width="90%" custom-class="max-w-md">
      <p class="mb-4 text-gray-500">这个功能应当只在你理解这里需要什么URL时使用</p>
      <el-input  type="textarea" :autosize="{minRows: 4, maxRows: 6}" placeholder="请输入带有身份认证信息的URL" v-model="state.urlInput" spellcheck="false"></el-input>
      <template #footer>
        <span class="dialog-footer">
          <el-button  @click="state.showUrlDlg = false" class="focus:outline-none">取消</el-button>
...
const setTitle = () => {
  document.title = `崩坏跃迁/原神祈愿 记录导出工具 - v${version}`
}
